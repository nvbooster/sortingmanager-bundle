...
class SortableColumnExtension extends \Twig_Extension
{
    ...
    public function renderColumnHeader(\Twig_Environment $env, Control $control, $name, $options = [])
    {
        if ($control->isColumnSortable($name)) {
            ...
            $label = $this->escape($env, $this->translator->trans($options['label'], [], $options['translation_domain']));
     ...
     * @return string
     */
    protected function buildLink($params = [])
    {
        $params = array_merge(
     ...
     * @param array $params
     */
    protected function plainparams($params = [], $parent = false)
    {
        $parts = [];
